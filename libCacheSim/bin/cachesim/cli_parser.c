...
unsigned long conv_size_str_to_byte_ul(char *cache_size_str);
...
static void init_arg(struct arguments *args) {
  ...
  for (int i = 0; i < N_MAX_ALGO_CACHE; i++) {
    args->algo_pair[i].algorithm = NULL;
    args->algo_pair[i].cache_size = 0;
  }
  args->n_algo_pair = 0;
}
...
void parse_cmd(int argc, char *argv[], struct arguments *args) {
  ...
  args->algo_pair[0].algorithm = strdup(args->args[2]);
  args->algo_pair[0].cache_size = conv_size_str_to_byte_ul(args->args[3]);

  args->algo_pair[1].algorithm = strdup(args->args[4]);
  args->algo_pair[1].cache_size = conv_size_str_to_byte_ul(args->args[5]);
  ...
  for (int i = 0; i < args->n_algo_pair; i++) {
    printf("L%d Cache: Algorithm=%s, Size=%lu\n",
           (i+1), args->algo_pair[i].algorithm, args->algo_pair[i].cache_size);

    args->caches[i] = create_cache(
        args->trace_path, args->algo_pair[i].algorithm,
        args->algo_pair[i].cache_size, args->eviction_params,
        args->consider_obj_metadata);

    if (args->admission_algo != NULL) {
      args->caches[i]->admissioner =
          create_admissioner(args->admission_algo, args->admission_params);
    }

    if (args->prefetch_algo != NULL) {
      args->caches[i]->prefetcher = create_prefetcher(
          args->prefetch_algo, args->prefetch_params,
          args->algo_pair[i].cache_size);
    }
  }
...
#define MAX_ALGO_LEN 4096
    ...
    while (str != NULL && str[0] != '\0') {
        /* Parse algorithms separated by commas */
        algo = strsep(&str, ",");
        if (n_algo >= N_MAX_ALGO_CACHE) {
            ERROR("Too many eviction algorithms specified, maximum is %d\n", N_MAX_ALGO_CACHE);
            exit(1);
        }

        args->algo_pair[n_algo].algorithm = strdup(algo);
        n_algo++;
    }

    args->n_algo_pair = n_algo; // Update the total number of algorithm-cache pairs
#undef MAX_ALGO_LEN
}
...
static int conv_cache_sizes(char *cache_size_str, struct arguments *args) {
    ...
    while (token != NULL) {
        if (n_pair_sizes >= N_MAX_ALGO_CACHE) {
            ERROR("Too many cache sizes specified, maximum is %d\n", N_MAX_ALGO_CACHE);
            exit(1);
        }
        ...
        if (strchr(token, '.') != NULL) {
            ...
            args->algo_pair[n_pair_sizes].cache_size = (uint64_t)(wss * atof(token));
        } else {
            args->algo_pair[n_pair_sizes].cache_size = conv_size_str_to_byte_ul(token);
        }
    ...
    if (n_pair_sizes == 1 && args->algo_pair[0].cache_size == 0) {
        set_cache_size(args, args->reader);
    }

    args->n_algo_pair = n_pair_sizes;
    return args->n_algo_pair;
}
...
#define N_AUTO_CACHE_SIZE 8
    ...
    for (int i = 0; i < N_AUTO_CACHE_SIZE; i++) {
        if (n_pair_sizes >= N_MAX_ALGO_CACHE) {
            ERROR("Too many auto-generated cache sizes, maximum is %d\n", N_MAX_ALGO_CACHE);
            exit(1);
        }

        if (args->ignore_obj_size) {
            if ((long)(wss_obj * s[i]) > 4) {
                args->algo_pair[n_pair_sizes].cache_size = (long)(wss * s[i]);
                n_pair_sizes++;
            }
        } else {
    ...
    args->n_algo_pair = n_pair_sizes; // Update the total number of pairs
#undef N_AUTO_CACHE_SIZE
}
...
#define OUTPUT_STR_LEN 1024
    ...
    int n = snprintf(
        ...
        "trace path: %s, trace_type %s, ofilepath %s, %d threads, warmup %d sec, total %d algo-cache pairs",
        args->trace_path, g_trace_type_name[args->trace_type], args->ofilepath,
        args->n_thread, args->warmup_sec, args->n_algo_pair);

    for (int i = 0; i < args->n_algo_pair; i++) {
        n += snprintf(output_str + n, OUTPUT_STR_LEN - n - 1, ", %s:%llu",
                      args->algo_pair[i].algorithm,
                      args->algo_pair[i].cache_size);
    }
