...
REGEX = r"(?P<path>[\w/.]+) (?P<algo>[\w-]+) cache size\s+(?P<size>\d+\w+),\s+(?P<nreq>\d+)\s+req, miss ratio\s+(?P<miss_ratio>\d+\.\d+)"
...
def run_cachesim_time(
    ...
    print("DEBUG: stdout_str =\n", stdout_str)
    for line in stdout_str.split("\n"):
        logger.debug("cachesim log: " + line)
        m = re.search(REGEX, line)
        if not m:
            continue
        ts_list.append(len(ts_list) * report_interval)  # 시간 데이터를 생성
